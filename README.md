...
1. **Canny Edge Detector의 도입**:
   현재의 `cv2.adaptiveThreshold` 대신 다단계 엣지 검출기인 Canny 알고리즘(`cv2.Canny`)을 사용할 수 있습니다. 가우시안 스무딩을 통한 노이즈 제거와 Hysteresis Thresholding 적용 과정을 거치면 불필요한 잔선을 억제하고 훨씬 정확하고 부드러운 만화 외곽선을 얻을 수 있습니다.
2. **Morphological Operations을 통한 마스크 정제**:
   추출된 엣지 마스크에 자잘한 점 형태의 노이즈가 남을 경우, 침식과 팽창을 결합한 열기`cv2.MORPH_OPEN` 연산을 적용하여 윤곽선의 굵기는 유지하면서 미세한 노이즈만 깔끔하게 제거할 수 있습니다
